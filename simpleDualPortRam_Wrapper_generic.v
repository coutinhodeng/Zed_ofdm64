// -------------------------------------------------------------
// 
// File Name: D:\OFDM- HDL\simulation\baseband\v8\src\Tx\Zed_ofdm64\simpleDualPortRam_Wrapper_generic.v
// Created: 2025-04-21 20:44:15
// 
// Generated by MATLAB 23.2, HDL Coder 23.2, and Simulink 23.2
// 
// -------------------------------------------------------------


// -------------------------------------------------------------
// 
// Module: simpleDualPortRam_Wrapper_generic
// Source Path: simpleDualPortRam_Wrapper_generic
// Hierarchy Level: 3
// Model version: 3.61
// 
// -------------------------------------------------------------

`timescale 1 ns / 1 ns

module simpleDualPortRam_Wrapper_generic
          (clk,
           reset,
           enb,
           enb_1_1_1,
           wr_din_re,
           wr_din_im,
           wr_addr,
           wr_en,
           rd_addr,
           dout_re,
           dout_im);

  parameter integer AddrWidth  = 1;
  parameter integer DataWidth  = 1;

  input   clk;
  input   reset;
  input   enb;
  input   enb_1_1_1;
  input   signed [DataWidth - 1:0] wr_din_re;  // parameterized width
  input   signed [DataWidth - 1:0] wr_din_im;  // parameterized width
  input   [AddrWidth - 1:0] wr_addr;  // parameterized width
  input   wr_en;  // ufix1
  input   [AddrWidth - 1:0] rd_addr;  // parameterized width
  output  signed [DataWidth - 1:0] dout_re;  // parameterized width
  output  signed [DataWidth - 1:0] dout_im;  // parameterized width


  wire const_one;
  reg  filwait;
  reg  enb_1_1_1_delay1;
  wire not_en1_delay1;
  wire filmode;
  reg  filreg;
  wire notfilreg;
  wire ram_is_fastest_rate;
  reg  enb_1_1_1_delay2;
  reg  enb_1_1_1_delay3;
  wire not_en1_delay3;
  wire not_en1;
  wire not_en1_delay2;
  wire not_en0;
  wire ram_is_down2;
  wire bypass2_sel;
  wire bypass1_and;
  wire ram_is_down3;
  wire bypass1_sel;
  reg signed [DataWidth - 1:0] wr_din_re_last_value;  // sfix32_En8
  wire signed [DataWidth - 1:0] wr_din_re_1;  // sfix32_En8
  wire signed [DataWidth - 1:0] wr_din_im_1;  // sfix32_En8
  reg signed [DataWidth - 1:0] wr_din_im_last_value;  // sfix32_En8
  reg [AddrWidth - 1:0] wr_addr_last_value;  // ufix10
  wire [AddrWidth - 1:0] wr_addr_1;  // ufix10
  reg  wr_en_last_value;  // ufix1
  wire wr_en_1;  // ufix1
  reg [AddrWidth - 1:0] rd_addr_last_value;  // ufix10
  wire [AddrWidth - 1:0] rd_addr_1;  // ufix10
  wire signed [DataWidth - 1:0] dout_re_1;  // sfix32_En8
  wire signed [DataWidth - 1:0] dout_im_1;  // sfix32_En8
  wire ram_is_down4;
  wire bypass1_en;
  reg signed [DataWidth - 1:0] dout_re_last_value;  // sfix32_En8
  wire signed [DataWidth - 1:0] dout_re_bypass_1;  // sfix32_En8
  reg signed [DataWidth - 1:0] dout_re_bypass_last_value;  // sfix32_En8
  wire signed [DataWidth - 1:0] dout_re_bypass_bypass;  // sfix32_En8
  reg signed [DataWidth - 1:0] dout_im_last_value;  // sfix32_En8
  wire signed [DataWidth - 1:0] dout_im_bypass_1;  // sfix32_En8
  reg signed [DataWidth - 1:0] dout_im_bypass_last_value;  // sfix32_En8
  wire signed [DataWidth - 1:0] dout_im_bypass_bypass;  // sfix32_En8


  assign const_one = 1;

  always @(posedge clk or posedge reset)
    begin : fil_wait
      if (reset) begin
        filwait <= 0;
      end
      else begin
        if (enb) begin
          filwait <= const_one;
        end
      end
    end

  always @(posedge clk or posedge reset)
    begin : ram_enb_delay1
      if (reset) begin
        enb_1_1_1_delay1 <= 1'b1;
      end
      else begin
        enb_1_1_1_delay1 <= enb_1_1_1;
      end
    end

  assign not_en1_delay1 = !enb_1_1_1_delay1;

  assign filmode = filwait && (not_en1_delay1 && enb_1_1_1 && enb);

  always @(posedge clk or posedge reset)
    begin : fil_hold
      if (reset) begin
        filreg <= 0;
      end
      else begin
        if (filmode) begin
          filreg <= const_one;
        end
      end
    end

  assign notfilreg = !filreg;

  assign ram_is_fastest_rate = enb_1_1_1 && enb && notfilreg;

  always @(posedge clk or posedge reset)
    begin : ram_enb_delay2
      if (reset) begin
        enb_1_1_1_delay2 <= 1'b1;
      end
      else begin
        enb_1_1_1_delay2 <= enb_1_1_1_delay1;
      end
    end

  always @(posedge clk or posedge reset)
    begin : ram_enb_delay3
      if (reset) begin
        enb_1_1_1_delay3 <= 1'b1;
      end
      else begin
        enb_1_1_1_delay3 <= enb_1_1_1_delay2;
      end
    end

  assign not_en1_delay3 = !enb_1_1_1_delay3;

  assign not_en1 = !enb_1_1_1;

  assign not_en1_delay2 = !enb_1_1_1_delay2;

  assign not_en0 = !enb;

  assign ram_is_down2 = (enb_1_1_1 && not_en1_delay1 && enb_1_1_1_delay2 && not_en1_delay3) || (not_en1 && enb_1_1_1_delay1 && not_en1_delay2 && enb_1_1_1_delay3) || (not_en1 && enb_1_1_1_delay1 && enb_1_1_1_delay2 && enb_1_1_1_delay3 && enb) || (enb_1_1_1 && not_en1_delay1 && enb_1_1_1_delay2 && enb_1_1_1_delay3 && not_en0);

  assign bypass2_sel = ram_is_fastest_rate || ram_is_down2 || filreg;

  assign bypass1_and = ram_is_down2 && enb_1_1_1;

  assign ram_is_down3 = (enb_1_1_1 && not_en1_delay1 && not_en1_delay2 && enb_1_1_1_delay3) || (not_en1 && enb_1_1_1_delay1 && not_en1_delay2 && not_en1_delay3) || (not_en1 && not_en1_delay1 && enb_1_1_1_delay2 && not_en1_delay3) || (not_en1 && not_en1_delay1 && enb_1_1_1_delay2 && enb_1_1_1_delay3 && enb);

  assign bypass1_sel = (ram_is_fastest_rate || bypass1_and || ram_is_down3) && notfilreg;

  always @(posedge clk or posedge reset)
    begin : wr_din_re_bypass
      if (reset) begin
        wr_din_re_last_value <= 0;
      end
      else begin
        if (enb_1_1_1) begin
          wr_din_re_last_value <= wr_din_re;
        end
      end
    end

  assign wr_din_re_1 = (ram_is_fastest_rate == 1 ? wr_din_re : wr_din_re_last_value);

  always @(posedge clk or posedge reset)
    begin : wr_din_im_bypass
      if (reset) begin
        wr_din_im_last_value <= 0;
      end
      else begin
        if (enb_1_1_1) begin
          wr_din_im_last_value <= wr_din_im;
        end
      end
    end

  assign wr_din_im_1 = (ram_is_fastest_rate == 1 ? wr_din_im : wr_din_im_last_value);

  always @(posedge clk or posedge reset)
    begin : wr_addr_bypass
      if (reset) begin
        wr_addr_last_value <= 0;
      end
      else begin
        if (enb_1_1_1) begin
          wr_addr_last_value <= wr_addr;
        end
      end
    end

  assign wr_addr_1 = (ram_is_fastest_rate == 1 ? wr_addr : wr_addr_last_value);

  always @(posedge clk or posedge reset)
    begin : wr_en_bypass
      if (reset) begin
        wr_en_last_value <= 0;
      end
      else begin
        if (enb_1_1_1) begin
          wr_en_last_value <= wr_en;
        end
      end
    end

  assign wr_en_1 = (ram_is_fastest_rate == 1 ? wr_en : wr_en_last_value);

  always @(posedge clk or posedge reset)
    begin : rd_addr_bypass
      if (reset) begin
        rd_addr_last_value <= 0;
      end
      else begin
        if (enb_1_1_1) begin
          rd_addr_last_value <= rd_addr;
        end
      end
    end

  assign rd_addr_1 = (ram_is_fastest_rate == 1 ? rd_addr : rd_addr_last_value);

  SimpleDualPortRAM_generic #(.AddrWidth(AddrWidth),
                              .DataWidth(DataWidth)
                              )
                            u_SimpleDualPortRAM_generic (.clk(clk),
                                                         .wr_din_re(wr_din_re_1),
                                                         .wr_din_im(wr_din_im_1),
                                                         .wr_addr(wr_addr_1),
                                                         .wr_en(wr_en_1),  // ufix1
                                                         .rd_addr(rd_addr_1),
                                                         .dout_re(dout_re_1),
                                                         .dout_im(dout_im_1)
                                                         );

  assign ram_is_down4 = (enb_1_1_1 && not_en1_delay1 && not_en1_delay2 && not_en1_delay3) || (not_en1 && enb_1_1_1_delay1 && not_en1_delay2 && not_en1_delay3) || (not_en1 && not_en1_delay1 && enb_1_1_1_delay2 && not_en1_delay3) || (not_en1 && not_en1_delay1 && not_en1_delay2 && enb_1_1_1_delay3) || !(enb_1_1_1 || enb_1_1_1_delay1 || enb_1_1_1_delay2 || enb_1_1_1_delay3) || (not_en1 && not_en1_delay1 && enb_1_1_1_delay2 && enb_1_1_1_delay3 && not_en0);

  assign bypass1_en = bypass1_and || (ram_is_down4 && enb_1_1_1_delay2) || (enb_1_1_1 && enb_1_1_1_delay1 && enb_1_1_1_delay2 && enb_1_1_1_delay3);

  always @(posedge clk or posedge reset)
    begin : dout_re_bypass
      if (reset) begin
        dout_re_last_value <= 0;
      end
      else begin
        if (bypass1_en) begin
          dout_re_last_value <= dout_re_1;
        end
      end
    end

  assign dout_re_bypass_1 = (bypass1_sel == 1 ? dout_re_1 : dout_re_last_value);

  always @(posedge clk or posedge reset)
    begin : dout_re_bypass_2
      if (reset) begin
        dout_re_bypass_last_value <= 0;
      end
      else begin
        if (enb) begin
          dout_re_bypass_last_value <= dout_re_bypass_1;
        end
      end
    end

  assign dout_re_bypass_bypass = (bypass2_sel == 1 ? dout_re_bypass_1 : dout_re_bypass_last_value);

  assign dout_re = dout_re_bypass_bypass;

  always @(posedge clk or posedge reset)
    begin : dout_im_bypass
      if (reset) begin
        dout_im_last_value <= 0;
      end
      else begin
        if (bypass1_en) begin
          dout_im_last_value <= dout_im_1;
        end
      end
    end

  assign dout_im_bypass_1 = (bypass1_sel == 1 ? dout_im_1 : dout_im_last_value);

  always @(posedge clk or posedge reset)
    begin : dout_im_bypass_2
      if (reset) begin
        dout_im_bypass_last_value <= 0;
      end
      else begin
        if (enb) begin
          dout_im_bypass_last_value <= dout_im_bypass_1;
        end
      end
    end

  assign dout_im_bypass_bypass = (bypass2_sel == 1 ? dout_im_bypass_1 : dout_im_bypass_last_value);

  assign dout_im = dout_im_bypass_bypass;

endmodule  // simpleDualPortRam_Wrapper_generic

